...
> does — SigV4-authenticated, **fourteen services and counting, each backed by a real datastore and
...
answers as AWS for **fourteen probe-proven services** — see
...
**Fourteen services are fronted, and every one is proven by a real-AWS-SDK compatibility probe** (`probe/aws-shim-*.sh`,
...
| **SSM Parameter Store** → Postgres + KMS-encrypted `SecureString` | **API Gateway** (HTTP API v2) → runtime proxy to Lambda | |
...
listed per service). Services the shim has **not** made faithful — Kinesis, Route 53, ECS/EKS, and the
rest of AWS — return an **honest `501`**, never a silent fake: **narrow and proven beats broad and
hand-wavy**. "Fourteen probe-proven services" is not "all of AWS"; it is a specific, verified surface.
