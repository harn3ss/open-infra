...
// K8sLoader lists every kind: Policy in the cluster and extracts its spec.dataPlane block
// (statements + appliesTo). Policies with no dataPlane block are ignored.
func K8sLoader(dc dynamic.Interface) Loader {
	return func(ctx context.Context) ([]PolicyDoc, error) {
		...
		for i := range list.Items {
			...
			doc := PolicyDoc{AppliesTo: toStrings(dp["appliesTo"])}
