...
| AppSync | managed GraphQL | **open-appsync** (resolver-first, VTL-faithful) | via the AWS-SDK shim's `appsync` service; opt-in `components.openAppsync`. Runtime behavior-faithful vs live AWS AppSync (captured goldens, CI-green); broader parity still experimental — see [aws-shim.md](aws-shim.md) |
...
| Fault Injection Simulator | chaos engineering | **Chaos Mesh** | `kind: FaultInjection`; blast-radius-scoped pod/network/stress/clock faults — see [chaos.md](chaos.md). Driven unattended nightly against the multi-master mesh by the [chaos suite](chaos-suite.md) |
