...
Resources:
  ...
  [gate] AppRole     AWS::IAM::Role                    -> Role
      ⚠ maps at plan only — NOT deployable (no create translator): kind: Role is a first-class assumable identity (trust policy + attached data-plane Policies; sts:AssumeRole on the aws-shim, #111), but there is no CFN create translator — author the Role + its trust natively.

Provisioning order:
  AppKey -> Assets -> AppRole -> Api -> Workflow

Provisionable, but review the caveats above — some mappings are lossy.
NOT DEPLOYABLE AS-IS: 1 resource(s) map at plan but have no create translator; `cfn deploy` is all-or-nothing and will refuse this stack until they are removed or expressed natively.
```
...
|---|---|---|
| `AWS::S3::Bucket` (+ `AWS::S3::BucketPolicy`) | `kind: Bucket` | a standalone MinIO bucket with versioning + lifecycle + object lock; an in-stack `BucketPolicy` collates in as `spec.policy` (public `*` principals only) |
...
| `AWS::EC2::Volume` | `kind: Volume` | `Size`→size, `MultiAttachEnabled`→shared (RWX), **`Encrypted`→LUKS at rest keyed by a customer `kind: EncryptionKey`** (`KmsKeyId` must reference an in-stack `AWS::KMS::Key`; destroying that key crypto-erases the volume). `AvailabilityZone`/`VolumeType`/`Iops`/`Throughput` are caveats (one flat cluster, Longhorn). **Refused**: `Encrypted` without a mappable `KmsKeyId` (no default-managed key is invented) and `SnapshotId` (an EBS snapshot id has no open-infra VolumeSnapshot to restore from). |
...
| `AWS::IAM::Policy`, `AWS::IAM::ManagedPolicy` | `kind: Policy` (`spec.dataPlane`) | The **data-plane** subset of an AWS policy document — `s3:*`/`dynamodb:*`/`lambda:*` actions on recognizable S3/DynamoDB/Lambda ARNs — imports into a real, enforced `kind: Policy` `spec.dataPlane` (Cedar-backed, applied at the aws-shim front doors; see [`policy-engine.md`](policy-engine.md)), attached to the policy's `Users`/`Groups`. ARNs map (`arn:aws:s3:::reports/*`→`Bucket::reports`, a table ARN→`Table::metrics`), `Allow`/`Deny` carry through faithfully. A `ManagedPolicy` translates the same way **when it carries inline `Groups`/`Users`/`Roles`**; a standalone one (attached elsewhere via `ManagedPolicyArns`) has no principal here and blocks. A **`Roles` attachment now translates** to `dataPlane.appliesTo: [Role::<name>]` and governs the assumed `sts:AssumeRole` session (the shim authenticates assumed-role sessions, #111; the engine governs `Role::` principals) — a declared caveat records that each named `kind: Role` must exist and carry a trust policy natively for a session to assume it. `ManagedPolicyName`/`Description`/`Path` are dropped metadata (caveats). **Refused** (never a silent partial — a dropped `Deny` or condition is a security hole, a dropped `Allow` a scope change): any action for a service with no data plane (`ec2:*`, `iam:*`), an unrecognizable ARN, any `Condition` (not importable yet — author it natively), a statement with no mappable `Resource`, and a policy attached to no principal. Enforced only where the shim front doors are enabled (opt-in); it never governs the k8s control plane (that stays RBAC + the boundary). |
| `AWS::ECS::Service` + `TaskDefinition` (+ `Cluster`) | `kind: Application` | **A collation** — a Service and its referenced TaskDefinition merge into one Application (Deployment+Service+Ingress+HPA). The primary container (the LB-named one, else the first with a port) → `image`/`port`/`env`; **additional containers → Application `sidecars`** (image/port/env). `DesiredCount`→a fixed replica count. A `Cluster` and a bare TaskDefinition are no-ops. **Refused**: a `Command`/`EntryPoint` override (any container), task `Volumes`, or a Service whose TaskDefinition isn't in the stack. Caveats: LoadBalancers / NetworkConfiguration / LaunchType, task Cpu/Memory / IAM roles, and per-sidecar CPU/memory/health checks (open-infra exposes via `domain`, sizes via quotas/HPA). |
| `AWS::DynamoDB::Table` | `kind: Table` | KeySchema+AttributeDefinitions→hashKey/rangeKey (S/N/B), `TimeToLiveSpecification`→ttlAttribute, **`GlobalSecondaryIndexes`→globalSecondaryIndexes** (name + key schema; backed by a real Mongo index — a Query with the index's name filters on its attributes; GSI Projection/throughput are caveats). `ProvisionedThroughput`/`BillingMode` are caveats (no capacity model); functions only where the aws-shim DynamoDB front door is enabled. **Refused**: local secondary indexes, `StreamSpecification`, `SSESpecification`, PITR/import — behavior-bearing with no faithful form. |
...
| `AWS::S3::Bucket` | `kind: Bucket` | A standalone MinIO bucket (the object-storage analog of `kind: Volume`). `BucketName`→bucket, `VersioningConfiguration`→versioning, `LifecycleConfiguration`→lifecycle rules (Id / Prefix / `ExpirationInDays` / `NoncurrentVersionExpiration.NoncurrentDays`), `ObjectLockConfiguration`→objectLock (WORM). An in-stack **`AWS::S3::BucketPolicy`** targeting this bucket **collates** in as `spec.policy` (applied verbatim via `put-bucket-policy`) — **public `*` principals only**; a Statement with an AWS-IAM-ARN principal **refuses** (MinIO can't resolve the ARN, so the policy would store but not enforce as written — same class as the db-stats/dataflow refuse-rather-than-apply rule). **Refused**: `BucketEncryption` (per-bucket SSE needs the opt-in objectEncryption stack — refusing beats an unencrypted bucket that looks encrypted), `CorsConfiguration` (a MinIO ceiling — see the ceilings table), a lifecycle `Transitions`/`ExpirationDate`/tag-or-size filter, and website/notifications/replication/public-access. `Tags` caveat. |
| `AWS::S3::BucketPolicy` | `kind: Bucket` (`spec.policy`) | **Collated** into its in-stack `AWS::S3::Bucket` — it has no standalone form. The `PolicyDocument` is serialized to compact JSON and applied verbatim via `put-bucket-policy`. **Public `*` principals only** (`"*"`, `{"AWS":"*"}`, `{"AWS":["*"]}`) — an AWS-IAM-ARN principal refuses (govern identity-scoped access via a data-plane `kind: Policy` instead). **Refused**: a `BucketPolicy` whose `Bucket` names/Refs a bucket not in this stack (a policy over an unmanaged bucket is not translatable). |
...
| `AWS::SNS::Topic` | `kind: Queue` (fanout) | `TopicName`→the stream. A NATS JetStream **Limits** stream (fan-out — every consumer reads each message). An inline **`lambda` `Subscription` to an in-stack Function maps** — the subscribing `kind: Function` gets the topic in `spec.queues` and reads the stream (see `AWS::SNS::Subscription` below). **Refused**: every other inline `Subscription` protocol (`sqs`/`http`/`email`/`sms`/…), `FifoTopic`, `KmsMasterKeyId`. |
| `AWS::SNS::Subscription` | `kind: Function` (`spec.queues`) | **Collated** into the subscribing Function — no standalone form. **Only `Protocol: lambda` to an in-stack `AWS::Lambda::Function` maps**: the topic's stream name is folded into that Function's `spec.queues`, so the Function consumes the topic's JetStream stream directly (open-infra's "subscribe a consumer to the topic" mechanism, in place of SNS push delivery). The `TopicArn` must `!Ref`/`!GetAtt` an in-stack `AWS::SNS::Topic` and the `Endpoint` an in-stack Function; both standalone `AWS::SNS::Subscription` resources and inline `Subscription` entries on a topic are honored the same way. **Refused**: an out-of-stack topic or endpoint (nothing we manage), an `sqs` subscription (open-infra builds no topic→queue bridge — instead give the consumer `spec.queues: [<topic>]` so it reads the topic stream directly), and `http`/`https`/`email`/`email-json`/`sms`/`application`/`firehose` (no open-infra outward push-delivery surface). |
| `AWS::SSM::Parameter` | `kind: Parameter` | The SSM Parameter Store equivalent — a value under a hierarchical path in Vault KV-v2, materialized into the namespace's Secret. `Name`→`path`, `Value`→`value`, `Type` (`String`/`SecureString`; `SecureString` is held encrypted at rest). A `StringList` stores its plain comma-separated string (no list-typed parameter — caveat). `Description`/`AllowedPattern`/`DataType`/`Tags` are advisory metadata with no equivalent (caveats). **Refused**: a lifecycle `Policies` (expiration/notification) — a Vault-backed parameter does not auto-expire, so silently dropping one would keep a value that should have expired. Consume it by adding `openinfra-parameters` to an Application/Function `spec.secrets`. |
...
|---|---|
| `AWS::IAM::Role`, `AWS::IAM::Group` | A `Role` is a first-class **runtime-assumable identity** (a trust policy + attached data-plane Policies; `sts:AssumeRole` issues temporary session credentials on the aws-shim, #111), and a data-plane `IAM::Policy`/`ManagedPolicy` carrying a `Roles` attachment translates and governs that assumed session (see the create-translators table). What has **no create translator** is the Role/Group resource itself: an inline role-policy document (`Allow`/`Deny` over `service:Action` on ARNs) has no faithful automatic form, the trust policy must be authored natively, and a `Group`'s control-plane grant can't be derived from its attached AWS policies. So both map at plan (`[gate]`) but are not create-deployable — author the `kind: Role`/`kind: Group` (and the Role's trust) natively. (A bare `IAM::User` identity maps; permissions attach via a Group's ClusterRole or a data-plane `IAM::Policy`.) |
...
| `AWS::S3::Bucket` `CorsConfiguration` | MinIO does **not** support per-bucket CORS via the S3 API — `PutBucketCors` (and `mc cors set`) returns `MalformedXML` on this release. MinIO configures CORS **globally** via the `MINIO_API_CORS_ALLOW_ORIGIN` env, not per bucket. The property refuses loudly (rather than silently dropping a rule the author expects to enforce); set CORS at the MinIO deployment instead. (The bucket's other properties still map — only `CorsConfiguration` refuses.) |
