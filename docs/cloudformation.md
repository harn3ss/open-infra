...
|---|---|
| `AWS::IAM::Role`, `AWS::IAM::Group` | An IAM policy document (`Allow`/`Deny` over `service:Action` on ARNs) has no faithful form in the RBAC control plane (`verb` on a `kind`, no `Deny`). Its **data-plane** part now translates — see `AWS::IAM::Policy`/`ManagedPolicy` in the create-translators table — but a `Role` (an assumed identity the shim can't authenticate as) and a `Group`'s control-plane grant remain unmappable. (A bare `IAM::User` identity maps; permissions attach via a Group's ClusterRole or a data-plane `IAM::Policy`.) |
